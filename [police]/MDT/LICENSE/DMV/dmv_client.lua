...
local kmh = 3.6
local VehSpeed = 0
local speed_limit_resi = 72.42048
local speed_limit_town = 72.42048
local speed_limit_freeway = 112.6541
local speed = kmh
...
Citizen.CreateThread(function()
    while true do
		...
		drawNotification("~o~DMV-SCHOOL: ~w~You are speeding! Slow down! You are driving in a ~o~45 mph~w~ zone!")
			Error = Error + 1	
			Citizen.Wait(10000)
		...
		drawNotification("~o~DMV-SCHOOL: ~w~You are speeding! Slow down! You are driving in a ~o~70 mph~w~ zone!")
			Error = Error + 1
			Citizen.Wait(10000)
		end
	end
