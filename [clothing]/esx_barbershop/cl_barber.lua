...
function OpenBarber()
	...
	if menuLoaded == false then
		barberShops = NativeUI.CreateMenu("", "Barbershop", 5, 100,"shopui_title_barber3","shopui_title_barber3",true)
...
function OpenShopMenuS(menu)
	local chev = _menuPool:AddSubMenu(menu, "Hairdressing","",true,true,true)
	local barb = _menuPool:AddSubMenu(menu, "Beards","",true,true,true)
	local maq = _menuPool:AddSubMenu(menu, "Makeup","",true,true,true)
...
function MakeupMenuFct(menu)
	...
	local makeupSelect = NativeUI.CreateListItem("Makeup", ppp, 0)
	local makeupBaseColor = NativeUI.CreateColourPanel("Primary color of makeup", Colors)
	local makeupHighColor = NativeUI.CreateColourPanel("Secondary color of makeup", Colors)
		local makeupOpacity = NativeUI.CreatePercentagePanel("0%", "Opacity of makeup", "100%")
			...
			ESX.TriggerServerCallback("Parow:GetCoiffeurMontant", function(result)
				if result then
					TriggerEvent('skinchanger:getSkin', function(skin)
						...
						ESX.ShowNotification("~g~New makeup purchased")
					end)
	   
				else
					ESX.ShowNotification("~r~Not enough money")
				end
			end)
		end
end
...
function BarbeMenuFct(menu)
	p = {"3-day beard","Goatee","Mamène","Goatee Style","Chin","Short beard","Collier","Unshaven","D'Artagnan","Moustache","Garnished","Unshavene 2","Goatee 2","Collier 2","Pronounced Collier","Traced Collier","Innovative","Rouflak","Prisoner","Mexican","Mexican Fournis","Mexican Imposant","Mexican Tombant","Mexican Classe","Mexican Garnished","Biker","Rouflak Chargée","Biker","Bearded"}
	local beardSelect = NativeUI.CreateListItem("Beard", p, 0)
	local beardOpacity = NativeUI.CreatePercentagePanel("0%", "Length", "100%")
	local beardBaseColor = NativeUI.CreateColourPanel("Primary beard color", Colors)
	local beardHighColor = NativeUI.CreateColourPanel("Secondary beard color", Colors)
		...
		ESX.TriggerServerCallback("Parow:GetCoiffeurMontant", function(result)
			if result then
				TriggerEvent('skinchanger:getSkin', function(skin)
					...
					skin.beard_4 = pp[4]
					TriggerServerEvent('esx_skin:save', skin)
					ESX.ShowNotification("~g~New beard shaven")
				end)
...
function HairCutMenuFct(menu)
	...
	local hairSelect = NativeUI.CreateListItem("Hairdressing", hairstyles, 0)
	local hairBaseColor = NativeUI.CreateColourPanel("Primary color", Colors)
	local hairHighColor = NativeUI.CreateColourPanel("Secondary color", Colors)
		...
		ESX.TriggerServerCallback("Parow:GetCoiffeurMontant", function(result)
			if result then
				TriggerEvent('skinchanger:getSkin', function(skin)
					...
					TriggerServerEvent('esx_skin:save', skin)
					ESX.ShowNotification("~g~New haircut done")
				end)
...
Citizen.CreateThread(function()
	...
	for i=1, #ConfigBarber.Shops, 1 do
		...
		AddTextComponentString("Hairdressing")
