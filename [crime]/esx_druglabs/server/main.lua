...
RegisterNetEvent('esx_druglabs:server:CanCarry')
AddEventHandler('esx_druglabs:server:CanCarry', function(amount)
    local sourcePlayer = ESX.GetPlayerFromId(source)

    if sourcePlayer.canCarryItem('methbad', amount) then
        bool(true)
    else
        bool(false)
    end
