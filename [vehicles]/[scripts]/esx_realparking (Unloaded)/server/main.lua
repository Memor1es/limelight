				...
				if type(rs) == 'table' and #rs > 0 then
					...
					MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND @plate = plate', {
						...
						if result[1] ~= nil then
							...
							if vehicleData.props.model == originalvehprops.model then
								...
								print(('esx_advancedgarage: %s attempted to Cheat! Tried Storing: '..vehicleData.props.model..'. Original Vehicle: '..originalvehprops.model):format(GetPlayerIdentifiers(source)[1]))
								cb({
									status = false,
									message = '車輛模型不符合',
								})
							...
							print(('esx_realparking: %s attempted to store an vehicle they don\'t own!'):format(GetPlayerIdentifiers(source)[1]))
							cb(false)
						end
					end)
				end
			...
			MySQL.Async.fetchAll("SELECT * FROM car_parking WHERE owner = @identifier AND plate = @plate", {
				...
				if type(rs) == 'table' and #rs > 0 and rs[1] ~= nil then
					...
					if parkingCard > 0 then
						fee = 0
						xPlayer.removeInventoryItem('parkingcard', 1)
					end
