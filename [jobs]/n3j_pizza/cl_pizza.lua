...
function StworzBlip()
	...
	BlipPizza = AddBlipForCoord(283.79, -969.24, 29.87)
		SetBlipSprite(BlipPizza, 436)
		SetBlipColour(BlipPizza, 81)
		SetBlipScale(BlipPizza, 0.6)
		SetBlipAsShortRange(BlipPizza, true)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('Drusillas Pizzeria')
		EndTextCommandSetBlipName(BlipPizza)
end	
...
function CreateMissionBlip(x,y,z)
	...
	AddTextEntry('MYBLIP', "Delivery destination")
...
function WezDostawe()
	...
	exports['mythic_notify']:DoCustomHudText('inform', 'You started delivery at a pizzeria, find the location on your GPS', 5000)
...
Citizen.CreateThread(function()
	while true do
	Citizen.Wait(0)
		for k,v in pairs(Config.PrzygotowywaniePizzy) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('[E] to ~g~ prepare pizza'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
						FreezeEntityPosition(PlayerPedId(), true)
								...
								local finished = exports["gen-taskbar"]:taskBar(10000,"Preparing pizza")
								Citizen.Wait(1000)
								FreezeEntityPosition(PlayerPedId(), false)
								ClearPedTasksImmediately(PlayerPedId())
								TriggerServerEvent('n3j_dodajpizze')
					end
				end
		end
	end
...
Citizen.CreateThread(function()
	while true do
	Citizen.Wait(0)
		for k,v in pairs(Config.PakowaniePizzy) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('[E] to ~g~ put into the oven and pack ~w~ the pizza'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
						FreezeEntityPosition(PlayerPedId(), true)
						ClearPedSecondaryTask(PlayerPedId())
						loadAnimDict( "mini@repair" ) 
						TaskPlayAnim( PlayerPedId(), "mini@repair", "fixing_a_ped", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
                        local finished = exports["gen-taskbar"]:taskBar(9700,"Packing the pizza into a box")
						Citizen.Wait(1850)
						FreezeEntityPosition(PlayerPedId(), false)
						ClearPedTasks(PlayerPedId())
						TriggerServerEvent('n3j_dodajspakowana')
					end
				end
		end
	end
	...
	Citizen.Wait(0)
		for k,v in pairs(Config.JobPoints) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('[E] to ~g~deliver ~w~ the pizza'))	
					...
					DrawText3D(v.x, v.y, v.z, ('[E] to ~r~cancel ~w~the delivery'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
						ClearPedSecondaryTask(PlayerPedId())
						loadAnimDict( "anim@heists@keycard@" ) 
						TaskPlayAnim( PlayerPedId(), "anim@heists@keycard@", "exit", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
						...
						ClearPedTasks(PlayerPedId())
						inMission = false
						exports['mythic_notify']:DoHudText('error', 'You have cancelled the delivery')
					end
				end
		end
	end
...
Citizen.CreateThread(function()
	...
	local d = GetDistanceBetweenCoords(pcoords, coords[1], coords[2], coords[3], true)
		if inMission == true and MissionCoords ~= false and d < 20.0 then
			DrawText3D(coords[1], coords[2], coords[3], '[E] to ~g~ deliver ~w~the pizza')
			if IsControlJustReleased(0, 38) and d < 5.0 then
				...
				local finished = exports["gen-taskbar"]:taskBar(3500,"Knocking on the door")
				...
				FreezeEntityPosition(PlayerPedId(), false)
				ClearPedTasks(PlayerPedId())
				TriggerServerEvent('n3j_wynagrodzenie')
			end
		end
	end
end)
