...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('[esx_ccdealer] [^3WARNING^7] Plate character count reached, %s/8 characters!'):format(char))
	end
end)
...
MySQL.ready(function()
	MySQL.Async.fetchAll('SELECT * FROM vehicle_categories', {}, function(_categories)
		...
		MySQL.Async.fetchAll('SELECT * FROM vehicles', {}, function(_vehicles)
			...
			TriggerClientEvent('esx_ccdealer:sendCategories', -1, categories)
			TriggerClientEvent('esx_ccdealer:sendVehicles', -1, vehicles)
		end)
	end)
end)
...
RegisterNetEvent('esx_ccdealer:setVehicleOwnedPlayerId')
AddEventHandler('esx_ccdealer:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
...
ESX.RegisterServerCallback('esx_ccdealer:getSoldVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT client, model, plate, soldby, date FROM vehicle_sold', {}, function(result)
		cb(result)
	end)
end)

RegisterNetEvent('esx_ccdealer:rentVehicle')
AddEventHandler('esx_ccdealer:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)
...
RegisterNetEvent('esx_ccdealer:getStockItem')
AddEventHandler('esx_ccdealer:getStockItem', function(itemName, count)
...
RegisterNetEvent('esx_ccdealer:putStockItems')
AddEventHandler('esx_ccdealer:putStockItems', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('esx_ccdealer:getCategories', function(source, cb)
	cb(categories)
end)

ESX.RegisterServerCallback('esx_ccdealer:getVehicles', function(source, cb)
	cb(vehicles)
end)

ESX.RegisterServerCallback('esx_ccdealer:buyVehicle', function(source, cb, model, plate)
...
ESX.RegisterServerCallback('esx_ccdealer:getCommercialVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT price, vehicle FROM cardealer_vehicles ORDER BY vehicle ASC', {}, function(result)
		cb(result)
	end)
end)

ESX.RegisterServerCallback('esx_ccdealer:buyCarDealerVehicle', function(source, cb, model)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('esx_ccdealer:returnProvider')
AddEventHandler('esx_ccdealer:returnProvider', function(vehicleModel)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.job.name == 'cardealer' then
		MySQL.Async.fetchAll('SELECT id, price FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			...
			if result[1] then
				...
				print(('[esx_ccdealer] [^3WARNING^7] %s attempted selling an invalid vehicle!'):format(xPlayer.identifier))
			end
		end)
	end
end)

ESX.RegisterServerCallback('esx_ccdealer:getRentedVehicles', function(source, cb)
...
ESX.RegisterServerCallback('esx_ccdealer:giveBackVehicle', function(source, cb, plate)
...
ESX.RegisterServerCallback('esx_ccdealer:resellVehicle', function(source, cb, plate, model)
	...
	if xPlayer.job.name == 'cardealer' then
		...
		if not resellPrice then
			print(('[esx_ccdealer] [^3WARNING^7] %s attempted to sell an unknown vehicle!'):format(xPlayer.identifier))
			cb(false)
		else
						...
						if result[1] then -- does the owner match?
							...
							if vehicle.model == model then
								if vehicle.plate == plate then
									...
									print(('[esx_ccdealer] [^3WARNING^7] %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
									cb(false)
								end
							else
								print(('[esx_ccdealer] [^3WARNING^7] %s attempted to sell an vehicle with model mismatch!'):format(xPlayer.identifier))
								cb(false)
							end
						end
					end)
...
ESX.RegisterServerCallback('esx_ccdealer:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		cb(inventory.items)
	end)
end)

ESX.RegisterServerCallback('esx_ccdealer:getPlayerInventory', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_ccdealer:isPlateTaken', function(source, cb, plate)
...
ESX.RegisterServerCallback('esx_ccdealer:retrieveJobVehicles', function(source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('esx_ccdealer:setJobVehicleState')
AddEventHandler('esx_ccdealer:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('[esx_ccdealer] [^3WARNING^7] %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)
...
function PayRent(d, h, m)
	...
	print('[esx_ccdealer] [^2INFO^7] Paying rent cron job started')
	...
	MySQL.Async.fetchAll('SELECT owner, rent_price, plate FROM rented_vehicles', {}, function(result)
		...
		print(('[esx_ccdealer] [^2INFO^7] Paying rent cron job took %s seconds'):format(elapsedTime))
	end)
end
